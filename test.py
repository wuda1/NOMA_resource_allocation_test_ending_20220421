...
if __name__ == '__main__':
    ...
    print('Average rewards using average selection:', np.mean(r_episode))
    r_episode_mean_average = np.reshape(np.mean(r_episode, axis=0), -1)
    plt.plot(100 * np.arange(len(r_episode_mean_average)), r_episode_mean_average)
    plt.xlabel('时隙(TS)')
    ...
    plt.xlabel('时隙(TS)')
    plt.ylabel('上行链路NOMA系统总和能量效率(bit/J)')
    plt.legend(["平均资源分配策略", "随机资源分配策略", "功率最大化资源分配策略"])
    plt.show()
